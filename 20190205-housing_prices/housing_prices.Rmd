...
```{r}
# identify states with highest diff_avg
highest_diff <- state_hpi %>%
  mutate(diff = price_index - us_avg) %>% 
  group_by(year, state) %>%
  summarize(us_avg = mean(us_avg),
            price_index = mean(price_index),
  ...
  mutate(segment = ifelse(diff_avg > 0, 'above', 'below')) %>%
  filter(state != 'DC') %>%
  slice(which.max(diff_avg)) %>%
  ungroup()

# identify states with lowest diff_avg
lowest_diff <- state_hpi %>%
  mutate(diff = price_index - us_avg) %>% 
  group_by(year, state) %>%
  summarize(us_avg = mean(us_avg),
            price_index = mean(price_index),
  ...
  mutate(segment = ifelse(diff_avg > 0, 'above', 'below')) %>%
  filter(state != 'DC') %>%
  slice(which.min(diff_avg)) %>%
  ungroup()

total <- rbind(highest_diff, lowest_diff)


total %>% 
  ggplot() +
  geom_col(aes(year, diff_avg, fill = state)) +
