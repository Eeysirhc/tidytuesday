...
calfire_incidents %>%
  ...
  labs(x = "",
       y = "",
       title = "$ Damage by Acres per year - adjusted for inflation (log10 scale)")
```
...
# convert original variable to character to allow for joining of data
calfire_frap$cause <-as.character(calfire_frap$cause)

calfire_frap %>% 
  select(year_, cause, state) %>%
  inner_join(cause_coding) %>%
  group_by(year_, description, state) %>%
  count() %>%
  ungroup() %>%
  ggplot(aes(state, description, fill = n)) + 
  geom_tile() +
